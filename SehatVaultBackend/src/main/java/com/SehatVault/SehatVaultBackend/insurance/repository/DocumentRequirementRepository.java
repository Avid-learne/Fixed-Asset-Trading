package com.SehatVault.SehatVaultBackend.insurance.repository;

import com.SehatVault.SehatVaultBackend.insurance.entity.DocumentRequirement;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;

import java.util.List;
import java.util.Optional;
import java.util.UUID;

@Repository
public interface DocumentRequirementRepository extends JpaRepository<DocumentRequirement, UUID> {
    Optional<DocumentRequirement> findByDocumentType(String documentType);
    List<DocumentRequirement> findByIsActiveAndIsMandatory(Boolean isActive, Boolean isMandatory);
}
