...
import com.SehatVault.SehatVaultBackend.hospital.service.HospitalStaffSettingsService;
...
@RestController
@RequestMapping("/api/hospital-staff/settings")
@RequiredArgsConstructor
@CrossOrigin(origins = "*")
public class HospitalStaffSettingsController {

    private final HospitalStaffSettingsService hospitalStaffSettingsService;
    ...
    public ResponseEntity<?> getSettings(Authentication authentication) {
        try {
            ...
            HospitalStaffSettingsResponse data = hospitalStaffSettingsService.getSettingsByEmail(authentication.getName());
    ...
    public ResponseEntity<?> updateSettings(
            ...
            @RequestBody UpdateHospitalStaffSettingsRequest request
    ) {
        try {
            if (authentication == null || authentication.getName() == null) {
                return ResponseEntity.status(401).body(error("Unauthorized"));
            ...
            HospitalStaffSettingsResponse data = hospitalStaffSettingsService
