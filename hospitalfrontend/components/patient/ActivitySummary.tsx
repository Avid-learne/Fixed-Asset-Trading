// Short overview: Summary card showing HT-focused activity totals.
...
export default function ActivitySummary({ transactions }: Props) {
  const totals = useMemo(() => {
    ...
    const totalHTTransfers = transactions.filter(t => t.token_type === 'HT').length
    const totalTransactions = transactions.length
    return { totalHT, totalHTTransfers, totalTransactions }
  }, [transactions])
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>HT transaction overview</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="p-4 border rounded-md">
            <div className="text-sm text-muted-foreground">Net HT Movement</div>
            <div className={`text-2xl font-bold mt-1 ${totals.totalHT >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              {totals.totalHT >= 0 ? '+' : ''}{totals.totalHT} HT
            </div>
          </div>
          <div className="p-4 border rounded-md">
            <div className="text-sm text-muted-foreground">HT Transactions</div>
            <div className="text-2xl font-bold mt-1">{totals.totalHTTransfers}</div>
          </div>
          <div className="p-4 border rounded-md">
